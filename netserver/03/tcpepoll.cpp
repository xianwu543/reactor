...
int main(int argc,char *argv[])
{
    ...
    if (bind(listenfd,servaddr.addr(),sizeof(struct sockaddr)) < 0 )
    {
        ...
        for (int ii=0;ii<infds;ii++)       // 遍历epoll返回的数组evs。
        {
            ...
            else if (evs[ii].events & (EPOLLIN|EPOLLPRI))   // 接收缓冲区中有数据可以读。
            {
                ...
                if (evs[ii].data.fd==listenfd)   // 如果是listenfd有事件，表示有新的客户端连上来。
                {
                    ...
                    int clientfd = accept4(listenfd,(struct sockaddr*)&peer,&len,SOCK_NONBLOCK);
                    // setnonblocking(clientfd);         // 客户端连接的fd必须设置为非阻塞的。
                    InetAddress clientaddr(peer);
